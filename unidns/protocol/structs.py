...
class DNSPacket():
	...
	def to_bytes(self):
		...
		t += len(self.Questions).to_bytes(2, byteorder = 'big', signed=False)
		t += len(self.Answers).to_bytes(2, byteorder = 'big', signed=False)
		t += len(self.Authorities).to_bytes(2, byteorder = 'big', signed=False)
		t += len(self.Additionals).to_bytes(2, byteorder = 'big', signed=False)
...
class DNSQuestion():
	def __init__(self):
		self.QNAME:DNSName  = None
		self.QTYPE:DNSType  = None
		self.QCLASS:DNSClass = DNSClass.IN
		self.QU:bool     = False #used in MDNS, tset it true for multicast
